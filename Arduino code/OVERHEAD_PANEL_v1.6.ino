...
// --- CLASSES SECTION --- //
  ...
  class SERVO {
    ...
    unsigned long lastUpdateServo_1 = millis(); // Last update of position
    ...
    public:
      ...
      void Update(int interval, int pos){
        ...
        updatePos = pos;
        if( (millis() - lastUpdateServo_1) > updateInterval ){  //time to update
          servo.write(updatePos);
          lastUpdateServo_1 = millis();
        } 
      }
  };
...
// --- VARIABLES SECTION --- //
 ...
 // *PANEL C5-4 VARIABLES*
  ...
  // *GENERIC VARIABLES*
    ...
    unsigned long lastUpdateServo_1, lastUpdateServo_2; //last update of switches state
...
void loop() {
  ...
  // *ALL SERVO DETACH*
    ...
    if( (millis() - lastUpdateServo_2) >= 300 && flag_7 == 1){ //time to update

      flag_7 = 0;
      SERVO_SELECTOR_ENGINE_STARTOR.Update(0, 105);
      lastUpdateServo_2 = millis();
      
    }

    if( (millis() - lastUpdateServo_1) >= 600 ){  //time to update
    ...
    if( (millis() - lastUpdateServo_2) >= 600 ){  //time to update
